...
def run():
    ...
    rand_radius = random.uniform(10, 100)
    ...
    noise_level = 0.0001
    # noise_level = random.gauss(1000, 1000)
    if circle_present:
        for frameN in range(40):
            ...
            noise_stim = psychopy.visual.NoiseStim(win=win, units='pix', size = (512, 512), mask = 'circle',  noiseType='Uniform')
            noise_stim.size = rand_radius
            noise_stim.pos = (x, y) 
            noise_stim.draw()
            win.flip()
