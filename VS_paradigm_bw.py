...
def run():
    ...
    image = Image.open(correct_path).convert('L')
    img = psychopy.visual.ImageStim(win=win, image=image,units="pix")
    ...
    # noise_level = 0.0001
    noise_level = random.gauss(10, 1000)
    if circle_present:
        for frameN in range(40):
            ...
            presented = Image.fromarray((presented * 255).astype(np.uint8))
            
            circle_stim = psychopy.visual.ImageStim(win=win, image=presented, mask='gauss')
            #noise_stim = psychopy.visual.NoiseStim(win=win, mask = 'gauss', noiseType='gabor')
